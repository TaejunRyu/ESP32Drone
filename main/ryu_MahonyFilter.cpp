...
namespace AHRS{
// 전역 또는 테스크 상단 선언
float q0 = 1.0f, q1 = 0.0f, q2 = 0.0f, q3 = 0.0f; // 쿼터니언
...
uint32_t boot_start_time = esp_log_timestamp();
void MahonyAHRSupdate(float gx, float gy, float gz, float ax, float ay, float az, float mx, float my, float mz,float dt) {
    ...
    if (is_booting){    
        ...
        if (esp_log_timestamp() - boot_start_time < 3000) {
            Kp = 20.0f; 
 ...
 */
void calibrate_mahony_initial_attitude(float ax, float ay, float az, float mx=0.0f, float my=0.0f, float mz=0.0f ) {
    // 1. 가속도계 정규화
    float a_norm = sqrtf(ax*ax + ay*ay + az*az);
...
/**
 * @brief 적분항만 초기화 (오류 누적 제거)
 * 
 * 사용 시기: 특정 축에서 계속 drift가 발생할 때 호출
 */
