...
void FailSafe::update_health(Event::fault_event_data_t* fault) {
    ...
    switch(fault->id) {
        case Event::FAULT_ID_IMU:{  
            ...
            // 에러가 발생하여 HOLD_MODE 상태로 전환 ( 이 곳에서도 시스템 모드전환 및 산태 프래그를 체크하여 현재모드 설정을 어떻게 할지)
            ENV::g_sys.hold_mode = ENV::flight_hold_mode::MODE_ERR_HOLD_MODE;  
            break;
        }
...
esp_err_t FailSafe::reinit_all_sensors()
{
    ...
    Sensor::ICM20948::Main().init_bus(Interface::createBIF(Driver::I2C::get_instance().get_bus_handle(),Sensor::ICM20948::ADDR_VCC));
    Sensor::ICM20948::Main().initialize();
    Sensor::ICM20948::Sub().init_bus(Interface::createBIF(Driver::I2C::get_instance().get_bus_handle(),Sensor::ICM20948::ADDR_GND));
