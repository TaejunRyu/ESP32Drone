...
public:
    ...
    // 데이터 읽기
    inline std::tuple<esp_err_t, std::array<float, 3>, std::array<float, 3>> read_raw_data();
    inline std::tuple<esp_err_t, std::array<float, 3>, std::array<float, 3>> read_with_offset();
    ...
    esp_err_t get_mag(float * mag) { memcpy(mag,_mag_data.data(),sizeof(_mag_data.data())); return ESP_OK; } // 지자계 데이터 접근자
private:
